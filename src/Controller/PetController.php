...
class PetController extends AbstractController
{
    ...
    {
        $payload = $request->request->all() ?: (json_decode($request->getContent(), true) ?? []);
        $recipientUserId = (int) ($payload['recipientUserId'] ?? 0);
        $message = trim((string) ($payload['message'] ?? ''));
        $sender = $this->getUser();

        if ($recipientUserId <= 0 || $recipientUserId === $sender->getId()) {
            return $this->json(['success' => false, 'error' => 'Invalid recipient.'], 400);
        }
        if ($message === '') {
        ...
        if (!$recipient instanceof User || !$recipient->getMainPet()) {
            return $this->json(['success' => false, 'error' => 'Recipient unavailable.'], 404);
        }

        $session = $request->getSession();
        $list = $this->readMetaverseCommunications($session->get('pet.metaverse.communications', []));
        ...
        $entry = [
            ...
            'fromName' => $sender->getFullName() ?: $sender->getEmail(),
            'toUserId' => $recipient->getId(),
            'toName' => $recipient->getFullName() ?: $recipient->getEmail(),
            'message' => $message,
        ];
        $list[] = $entry;
        $session->set('pet.metaverse.communications', $this->compactMetaverseCommunications($list));
    ...
    {
        $user = $this->getUser();
        $all = $this->readMetaverseCommunications($request->getSession()->get('pet.metaverse.communications', []));
        $filtered = array_values(array_filter($all, static function (array $row) use ($user): bool {
            $fromUserId = (int) ($row['fromUserId'] ?? 0);
            $toUserId = (int) ($row['toUserId'] ?? 0);
            return $fromUserId === $user->getId() || $toUserId === $user->getId();
        }));

        usort($filtered, static fn (array $a, array $b) => (int) ($b['ts'] ?? 0) <=> (int) ($a['ts'] ?? 0));
        $filtered = array_slice($filtered, 0, 40);
        $rows = array_map(static function (array $row) use ($user): array {
            $row['direction'] = ((int) ($row['toUserId'] ?? 0) === $user->getId()) ? 'in' : 'out';
            return $row;
        }, $filtered);
     ...
     */
    private function readMetaverseCommunications(array $raw): array
    {
        $now = time();
        ...
        foreach ($raw as $item) {
            ...
            $rows[] = [
                ...
                'fromUserId' => (int) ($item['fromUserId'] ?? 0),
                'fromName' => (string) ($item['fromName'] ?? 'Unknown'),
                'toUserId' => (int) ($item['toUserId'] ?? 0),
                'toName' => (string) ($item['toName'] ?? 'Unknown'),
     ...
     * @return array<int,array<string,mixed>>
     */
    private function compactMetaverseCommunications(array $rows): array
    {
        usort($rows, static fn (array $a, array $b) => (int) ($b['ts'] ?? 0) <=> (int) ($a['ts'] ?? 0));
